...
const FoodCard = ({ n, t, a }: { n: string; t: string; a: string }) => (
  <div className="bg-white p-5 rounded-[2rem] border-2 border-[#E5E0D8] shadow-sm mb-4 text-left">
    ...
    <div className="flex space-x-2">
      ...
      <a 
        href={`https://www.google.com/maps/search/?api=1&query=$$?q=${encodeURIComponent(a)}`} 
        target="_blank" 
        rel="noreferrer" 
...
// --- 2. 資料庫：100% 精準對齊 4 日行程內容 ---
const scheduleData = [
  { day: "27", b: "標記", l: "水頭金道地", d: "東門餐廳", items: [
    { t: "06:00", title: "松山機場 (第二航廈)", note: "立榮航空櫃檯集合", map: "松山機場第二航廈" },
    { t: "07:00", title: "台北-金門 (B7-8801)", note: "07:00 起飛 / 08:05 抵達", map: "金門尚義機場" },
  ...
  { day: "29", b: "民宿", l: "談天樓", d: "新天地餐廳", items: [
    ...
    { t: "19:30", title: "瓊林聚落夜拍", note: "低光影下古厝建築的層次感", map: "瓊林聚落" }
  ]},
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F0] pb-36 font-sans text-[#3D3A36] text-left">
      <header className="p-8 pt-12 flex justify-between items-end">
        <div><p className="text-[#4A6741] font-bold text-[10px] uppercase text-left">May 2026</p><h1 className="text-3xl font-black text-left">金門迎城隍</h1></div>
        {tab === '帳單' && (
      ...
      <main className="px-6 text-left">
            ...
            {curr.items.map((it, i)=>(
              <div key={i} className="bg-white p-6 rounded-[2.5rem] border-2 border-[#E5E0D8] shadow-sm mb-4 text-left">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <a href={`https://www.google.com/maps/search/?api=1&query=$$?q=${encodeURIComponent(it.map)}`} target="_blank" rel="noreferrer" className="bg-[#E9F0EA] text-[#4A6741] px-3 py-1 rounded-full text-xs font-black">🗺️ 導航</a>
                </div>
                <h2 className="text-xl font-black leading-tight mb-2 text-[#3D3A36] text-left">{it.title}</h2>
                <div className="bg-[#F8F5F0] p-4 rounded-2xl border border-dashed border-[#8C8579]/30 text-sm italic text-[#3D3A36] text-left">💡 {it.note}</div>
              </div>
            ))}
            ...
            {day !== "30" && (
              <div className="mt-12 pt-8 border-t-2 border-dashed border-[#E5E0D8]">
                ...
                <div className="bg-white p-6 rounded-[2.5rem] border-2 border-[#4A6741] shadow-sm text-left">
                  ...
                  <a href={`https://www.google.com/maps/search/?api=1&query=$$?q=${encodeURIComponent("金門縣金湖鎮瓊林150號")}`} target="_blank" rel="noreferrer" className="text-xs italic text-[#4A6741] mb-1 underline block">📍 金門縣金湖鎮瓊林150號</a>
        ...
        {tab === '美食' && (
          <div className="space-y-4 text-left">
            <p className="text-[10px] font-black text-[#8C8579] uppercase mb-4 tracking-widest text-left">金門推薦美食</p>
