...
// 精確引入下方 JSX 會用到的所有圖示，徹底消滅 unused 警告
...
const schedule: DayData[] = [
  ...
  },
  {
    ...
    items: [
      { t: "09:00", title: "遇龍河三橋 (富里、金龍、遇龍)", desc: "拍攝遇龍河上著名的古石橋。水流平緩，兩岸峰林清秀，竹木繁茂。石橋與倒影相連酷似滿月，是拍攝田園山水的絕佳地點。", icon: "camera" },
      { t: "14:30", title: "灕江精華 (興坪-九馬畫山)", desc: "搭乘竹筏拍攝著名的「九馬畫山」。江面寬闊如鏡，奇峰倒影清晰，是灕江最精華的喀斯特山體景觀。", icon: "camera" },
      { t: "17:00", title: "興坪漁火專場攝影 (含漁翁)", desc: "特別安排漁民現場示範。在灕江暮色下，捕捉竹筏、魚鷹、煤油燈火與漁翁的神秘人文神采，是桂林最具代表性的經典大片。", note: "包含模特漁翁與魚鷹費用，拍攝約 2 小時", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "09:00", title: "陽朔西街人文 / 陽朔公園", desc: "西街具備 1400 多年歷史，明清風格建築與中西合璧的異國風情交織。適合捕捉人文寫實與古鎮氣息。", icon: "camera" },
      { t: "16:00", title: "烏龍泉夕陽攝影", desc: "烏龍泉是田園光影的頂級拍攝地。山峰排列有序，蜿蜒的灕江田園與群峰在夕陽下層次分明，捕捉如詩如畫的牧歌景象。", note: "建議使用腳架進行長曝，注意防蚊", icon: "camera" },
      { t: "19:00", title: "陽朔大橋夜拍", desc: "捕捉灕江夜晚的寧靜美感，或選擇觀賞印象劉三姐大型實景演出。", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "05:00", title: "相公山日出攝影", desc: "登上相公山遠眺，灕江在此處呈大 U 型迴轉。雲海、日出灑在群峰間的壯麗線條，呈現視覺震撼的層次感。", icon: "camera" },
      { t: "13:00", title: "黃洛瑤寨 / 長髮表演", desc: "探訪紅瑤族聚居地，拍攝「天下第一長髮村」的民俗生活。紅瑤族女性保留長髮習俗，具有極強的人文攝影價值。", icon: "camera" },
      { t: "15:00", title: "龍脊梯田 (九龍五虎 / 七星伴月)", desc: "拍攝「九龍五虎」—指九條山脊與五個山頭；「七星伴月」—指七個小山包守護彎月狀水田。線條感極強。", note: "換乘景區車，行李建議精簡", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "05:30", title: "龍脊平安壯寨 1 號點日出", desc: "捕捉清晨第一道曙光灑在梯田水面上的反射。薄霧繚繞中的壯族吊腳樓與階梯狀田壟，構成如詩如畫的晨霧奇觀。", icon: "camera" },
      { t: "14:00", title: "前往兩江四湖環城水系", desc: "灕江、桃花江、榕湖、杉湖等構成的環城水系，展現「城在景中、景在城中」的桂林山水美景。", icon: "camera" },
      { t: "19:00", title: "日月雙塔夜拍", desc: "拍攝座落在杉湖的金塔與銀塔。雙塔璀璨倒影與象鼻山遙遙相望，是桂林地標性夜景，適合慢門長曝。", note: "建議使用三腳架捕捉精確倒影", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "09:34", title: "桂林西 - 廣州南 (動車 G2231)", desc: "搭乘動車返回珠海。途經珠海情侶路，可遠眺日月貝大劇院。帶著滿滿的作品準備結束遠征行程。", icon: "train" },
      { t: "20:50", title: "澳門機場 (星宇 JX206)", desc: "搭機返抵桃園。攝影是凝固的時間，這趟行程紀錄了灕江煙雨與龍脊的脊樑。", note: "4/21 記得最後確認售票狀況", icon: "plane" }
    ]
  }
];
    ...
    <div className="min-h-screen bg-[#F1F5F2] text-[#2D3A30] font-sans antialiased pb-36">
      ...
      <header className="pt-12 px-6 pb-6 text-center">
        ...
        <div className="mt-5 flex justify-center">
           <div className="bg-white/90 px-5 py-2 rounded-full border border-[#D1DDD5] flex items-center gap-3 shadow-sm hover:border-[#2D5A27] transition-colors">
      ...
      {tab === 'plan' ? (
        <main className="px-6 space-y-8 animate-in fade-in duration-500">
          <div className="text-center">
            ...
            <div className="text-[#2D5A27] font-bold tracking-[0.2em] mt-2 opacity-80 uppercase text-[10px] bg-[#2D5A27]/5 py-1 rounded inline-block px-4">{c.location}</div>
          </div>
          ...
          {/* Timeline Items with Full Detailed Descriptions */}
        ...
        <main className="px-6 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          <div className="text-center py-8">
            ...
            <div className="text-[10px] text-[#A0B0A5] font-black tracking-[0.4em] uppercase mt-2">Ready to Go</div>
          </div>
          {gearData.map((g, i) => (
            <div key={i} className="bg-white rounded-[2.2rem] p-7 border border-[#D1DDD5] shadow-sm hover:border-[#2D5A27] transition-colors">
      ...
      {/* Bottom Floating Nav Bar (Jinmen Style) */}
      <div className="fixed bottom-10 left-12 right-12 flex justify-center z-50">
        <div className="bg-[#1A2E1F]/90 backdrop-blur-xl px-2 py-2 rounded-full flex gap-1 shadow-[0_25px_60px_rgba(0,0,0,0.4)] border border-white/10">
