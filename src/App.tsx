...
const schedule: DayData[] = [
  {
    ...
    items: [
      { t: "08:05", title: "桃園-澳門 (星宇 JX201)", desc: "集合搭機前往澳門。抵達後經由橫琴/拱北口岸前往珠海，隨後轉乘動車前往桂林。桂林以喀斯特地貌聞名，其「山青、水秀、洞奇、石美」是全球攝影師的夢幻題材。", icon: "plane" },
      { t: "16:39", title: "廣州南-桂林西 (動車 D1862)", desc: "抵達桂林西站後入住喜來登飯店。這座山水城市由灕江貫穿，城景交融，感受千年古城的悠閒氣息。", note: "需提早 1 小時到達車站安檢", icon: "train" }
    ]
  },
  {
    ...
    items: [
      ...
      { t: "14:30", title: "灕江精華 (興坪-九馬畫山)", desc: "搭乘竹筏拍攝著名的「九馬畫山」。江面寬闊如鏡，奇峰倒影清晰，捕捉灕江最精華的喀斯特山體層次感。", icon: "camera" },
      { t: "17:00", title: "興坪漁火專場攝影 (含漁翁)", desc: "特別安排漁民現場示範。在灕江暮色下，利用竹筏、魚鷹、煤油燈火與漁翁交互作用，捕捉傳統漁事活動的神秘神采。", note: "包含模特漁翁與魚鷹費用，拍攝約 2 小時", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "09:00", title: "陽朔公園 / 西山 / 西街人文", desc: "西街具備 1400 多年歷史，明清風格建築與異國風情交織。適合捕捉人文寫實與古鎮氣息。", icon: "camera" },
      { t: "14:00", title: "前往烏龍泉攝影基地", desc: "烏龍泉是田園光影的頂級拍攝地，山峰排列有序，捕捉蜿蜒的灕江與田園村落。", icon: "camera" },
      { t: "16:00", title: "烏龍泉夕陽攝影", desc: "捕捉夕陽下群峰層次分明的牧歌景象。光影在群峰間跳躍，是攝影愛好者的必選之地。", note: "建議使用腳架進行長曝，注意防蚊", icon: "camera" },
      { t: "19:00", title: "陽朔大橋夜拍 / 印象劉三姐", desc: "可選擇拍攝大橋餘輝，或觀看張藝謀導演的大型山水實景演出。", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "05:00", title: "相公山日出攝影", desc: "登上相公山遠眺，灕江在此處呈大 U 型迴轉。捕捉雲海、日出灑在群峰間的壯麗線條。", icon: "camera" },
      { t: "13:00", title: "黃洛瑤寨 / 長髮表演", desc: "探訪紅瑤族聚居地，拍攝「天下第一長髮村」的民俗生活，感受大山深處的人文魅力。", icon: "camera" },
      { t: "16:00", title: "平安壯寨 (九龍五虎 / 七星伴月)", desc: "拍攝「九龍五虎」—指九條山脊與五個山頭；「七星伴月」—指七個小山包守護彎月狀水田。線條感極強。", note: "換乘景區車，行李建議精簡", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      ...
      { t: "14:00", title: "兩江四湖環城水系", desc: "灕江、桃花江、榕湖、杉湖構成的環城水系，展現「城在景中、景在城中」的美景。", icon: "camera" },
      { t: "19:00", title: "日月雙塔夜拍", desc: "拍攝杉湖中的金塔與銀塔。雙塔璀璨倒影與象鼻山遙遙相望，是桂林地標性夜景，適合慢門長曝。", note: "建議使用三腳架捕捉精確倒影", icon: "camera" }
    ]
  },
  {
    ...
    items: [
      { t: "09:34", title: "桂林西 - 廣州南 (動車 G2231)", desc: "搭乘動車返回珠海。途經情侶路，可遠眺日月貝大劇院。帶著滿滿的作品準備結束攝影行程。", icon: "train" },
      { t: "20:50", title: "澳門機場 (星宇 JX206)", desc: "搭機返抵桃園。紀錄了灕江煙雨與龍脊的脊樑，結束這趟洗滌心靈的山水之旅。", note: "4/21 記得最後確認售票狀況", icon: "plane" }
    ]
  }
];
...
export default function App() {
  ...
  // 強制斷言，確保 TypeScript 認可索引安全，消滅最後一個紅字
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F2] text-[#2D3A30] font-sans antialiased pb-36">
      ...
      <header className="pt-12 px-6 pb-6 text-center">
        <div className="flex justify-center mb-1"><Leaf className="w-4 h-4 text-[#2D5A27] opacity-60 animate-pulse" /></div>
        ...
        <div className="mt-5 flex justify-center">
           <div className="bg-white/90 px-5 py-2 rounded-full border border-[#D1DDD5] flex items-center gap-3 shadow-sm">
             ...
             <span className="text-[12px] font-bold text-[#4A5D50]">導遊：曾克儉 13977316816</span>
           </div>
        </div>
      </header>
      ...
      {tab === 'plan' ? (
          ...
          <div className="bg-[#2D5A27] text-white p-7 rounded-[2.8rem] shadow-2xl space-y-6 border-b-[6px] border-[#1A3A1A]">
             ...
             <div className="grid grid-cols-3 gap-2 pt-1 text-center items-center">
               <div className="flex flex-col items-center border-r border-white/10">
                  <div className="text-[8px] opacity-40 mb-1 uppercase tracking-tighter">早餐</div>
               ...
               <div className="flex flex-col items-center border-r border-white/10 px-1">
                  ...
                  <div className="text-[8px] opacity-40 mb-1 uppercase tracking-tighter">午餐</div>
                  <div className="text-[12px] font-bold">{c.meals.l}</div>
               </div>
               <div className="flex flex-col items-center">
                  <div className="text-[8px] opacity-40 mb-1 uppercase tracking-tighter">晚餐</div>
                  <div className="text-[12px] font-bold">{c.meals.d}</div>
               </div>
             </div>
          </div>

          {/* Timeline Items (Showing Every Single Part) */}
        ...
        <main className="px-6 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          <div className="text-center py-8">
            ...
            <div className="text-[10px] text-[#A0B0A5] font-bold tracking-[0.3em] uppercase mt-2">Ready for Mission</div>
          </div>
          {gearData.map((g, i) => (
